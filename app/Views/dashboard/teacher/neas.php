<?php
$pageTitle = 'پنل شخصی- اخبار - رویداد - اطلاعیه';
...
<div class="col-md-10 offset-md-2 p-4">
  <h3 class="text-primary">اخبار | رویدادها | اطلاعیه ها</h3>
  <a href="/panel/neas/create" class="btn btn-primary w-100 d-block my-1">
    افزودن
  </a>
  <div class="container">
    <table class="table table-bordered table-hover table-striped">
      ...
      <tbody id="itemsTableBody">
        <?php if (empty($items)): ?>
          <tr>
            <td colspan="6" class="text-center">هیچ مطلبی یافت نشد.</td>
          </tr>
        <?php else: ?>
          <?php foreach ($items as $index => $item): ?>
            <tr id="item-row-<?= $item['id'] ?>">
              ...
              <td><?= htmlspecialchars(mb_substr($item['content'], 0, 100)) ?></td>
              <td>
                <?php
                $categoryLabels = [
                  'news' => 'خبر',
                ...
                echo $categoryLabels[$item['category']] ?? $item['category'];
                ?>
              </td>
              <td><?= toJalali($item['created_at'], 'Y/m/d') ?></td>
              <td class="">
                <a href="/panel/neas/delete/<?= $item['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('آیا از حذف این مطلب مطمئن هستید؟')">حذف</a>
              </td>
            </tr>
          <?php endforeach; ?>
        <?php endif; ?>
...
<script>
  ...
  function renderTable(items, page, perPage) {
    ...
    if (!items || items.length === 0) {
      tbody.innerHTML = `<tr><td colspan="6" class="text-center">هیچ مطلبی یافت نشد.</td></tr>`;
      return;
    }
    ...
    items.forEach((item, idx) => {
      ...
      html += `
        <tr id="item-row-${item.id}">
          ...
          <td>${escapeHtml(item.created_at_fa)}</td>
          <td>
            <a href="/panel/neas/delete/${item.id}" class="btn btn-danger" onclick="return confirm('آیا از حذف این مطلب مطمئن هستید؟')">حذف</a>
          </td>
        </tr>
      `;
    });
  ...
  function loadPage(page) {
    ...
    fetch(`/api/neas/list?page=${page}`)
      ...
      .catch(err => {
        ...
        tbody.innerHTML = `<tr><td colspan="8" class="text-center">خطا در بارگذاری داده‌ها</td></tr>`;
      });
  }
